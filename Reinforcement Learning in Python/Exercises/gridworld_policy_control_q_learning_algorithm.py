...
def epsilon_greedy_action_selection(
    ...
    Args:
        Q (ActionValueTable): The Q-table representing the action-value mappings for each
            state and action pair.
        state (State): The current state for which an action must be selected.
        epsilon (float): The probability of selecting a random action (exploration factor).
    ...
    Raises:
        ValueError: If epsilon is not between 0 and 1, or if there are no available
            actions for the given state.
        KeyError: If the specified state is not found in the Q-table.
    """
    if not 0 <= epsilon <= 1:
...
def q_learning(g: WindyGridworld,
          epsilon: float,
          alpha: float,
          gamma: float,
...
def main():
    ...
    rewards_per_episode, Q, update_counts = q_learning(g, epsilon=0.1, alpha=0.5, gamma=0.9, num_episodes=10_000)
