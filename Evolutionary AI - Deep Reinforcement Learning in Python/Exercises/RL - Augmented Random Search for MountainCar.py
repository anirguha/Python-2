...
"""MountainCar ARS training script.

Interactive Window / notebook usage:

    from importlib.util import module_from_spec, spec_from_file_location
    from pathlib import Path

    script_path = Path(
        "/Users/AnirbanGuha/Library/CloudStorage/GoogleDrive-guhaa1@gmail.com/My Drive/GitHub/Python-2/Evolutionary AI - Deep Reinforcement Learning in Python/Exercises/RL - Augmented Random Search for MountainCar.py"
    )

    spec = spec_from_file_location("mountaincar_ars", script_path)
    mountaincar_ars = module_from_spec(spec)
    spec.loader.exec_module(mountaincar_ars)

    results = mountaincar_ars.main([
        "--iterations", "20",
        "--population-size", "8",
        "--processes", "4",
        "--eval-episodes", "5",
...
def get_env_dimensions(env_name):
    ...
    try:
        obs_dim = env.observation_space.shape[0] if env.observation_space.shape else 1
        return obs_dim, env.action_space.n
    finally:
        env.close()
...
def get_env_bounds(env_name):
    env = gym.make(env_name)
    try:
        return float(getattr(env.unwrapped, 'min_position', -1.2)), float(getattr(env.unwrapped, 'goal_position', 0.6))
    finally:
        env.close()
...
def main(argv=None):
    ...
    with Pool(processes=args.processes) as pool:
        reward_per_iteration, final_params = evolution_strategy(
            initial_params,
            args.iterations,
            args.learning_rate,
    ...
    return {
        "reward_history": reward_per_iteration,
