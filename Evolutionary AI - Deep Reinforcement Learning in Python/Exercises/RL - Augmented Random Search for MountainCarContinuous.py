...
SCRIPT_DIR = Path("/Users/AnirbanGuha/Library/CloudStorage/GoogleDrive-guhaa1@gmail.com/My Drive/GitHub/Python-2/Evolutionary AI - Deep Reinforcement Learning in Python/Exercises")
...
def get_env_dimensions(env_name):
    ...
    try:
        ...
        return obs_dim, env.action_space.n # type: ignore
    finally:
        env.close()
...
def make_env(env_name, record=False, video_root=VIDEO_DIR):
    if record:
        ...
        video_folder = video_root / f"mountaincarContinuous-{timestamp}-{uuid4().hex[:8]}"
...
plt.plot(reward_per_iteration) #type: ignore
plt.title("MountainCar ARS Training Reward")
